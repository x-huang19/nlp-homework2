...
class FNNLM(nn.Module):
    ...
    def __init__(self, vocab_size, embedded_size, context_size, batch_size):
        ...
        self.linear1 = nn.Linear(context_size * embedded_size, embedded_size * 3)
        self.linear2 = nn.Linear(embedded_size * 3, vocab_size)
        #Drop out function
...
def train(model, dataloader_train, loss_function, optimizer, device, batch_size, epoch, dataloader_valid, scheduler, vocab_size, file1):
    ...
    for epochnum in range(1, epoch + 1):
        ...
        for index, text in enumerate(dataloader_train):
            ...
            counter += 1
            train_acc_num += (log_probs.argmax(dim=1) == label).sum().item()
            train_ann_counter += batch_size
    ...
    #Evaluating model with validation data
        file1.write("\n")
        file1.write('| epoch {:3d} train_acc {}'.format(epochnum, train_acc_num/train_ann_counter ))
        ...
        ppl = math.exp(total_loss_eval / total_count)
        ...
        torch.save(model, "modelfnn_{}_{}".format(epochnum, vocab_size))
...
if __name__ == "__main__":
    ...
    if not processed:
        ...
        for m in range(trainsize):
            # 首先应该对数据进行清洗
            # sentence = re.sub(r'[^\u4e00-\u9fa5，。]', '', context[m])# 仅保留中文 逗号句号
            sentence = context[m]
            cutwords = jieba.lcut(sentence)
            for subword in cutwords:
                if subword not in filterwords: # 去除一些噪声词 
        ...
        for m in range(trainsize):
            ...
            for n in range(len(words) - 3):
                trigram.append(([words[n],words[n + 1],words[n + 2]],words[n + 3])) # 3-gram
        ...
        with open('./fnnlm/trigram.pickle','wb') as datafile:
            pickle.dump(trigram, datafile)
            
        with open('./fnnlm/eval_trigram.pickle','wb') as datafile:
            pickle.dump(eval_trigram, datafile)
        
        with open('./fnnlm/tokens.pickle','wb') as datafile:
        ...
        with open('./fnnlm/trigram.pickle','rb') as datafile:
            trigram = pickle.load(datafile)
            
        with open('./fnnlm/eval_trigram.pickle','rb') as datafile:
            eval_trigram = pickle.load(datafile)
        
        with open('./fnnlm/tokens.pickle','rb') as datafile:
            tokens = pickle.load(datafile)  
        
        vocab_size = len(tokens.keys())        
    
    # trainingset = len(trigram)
    # unkcounter = 0
    # mycounter = {}
    # for data in trigram:
    #     label = data[1]
    #     if label not in mycounter.keys():
    #         mycounter[label] = 1
    #     else:
    #         mycounter[label] += 1
    # sorted_counter = sorted(mycounter.items(), key=lambda x:x[1], reverse = True)
    # print(dict(sorted_counter))
    # part3 training   
    testflag = 1
    
    if testflag:
        model = torch.load('./modelfnn_10_10424')
    else:
        model = FNNLM(vocab_size, embedded_size, context_size, batch_size).to(device) 
    
    
    file1 = open("log_{}.txt".format(vocab_size), "a")
    file1.write('lr:{} | batch_size:{} | embedded_size:{} \n'.format(lr, batch_size, embedded_size))
    ...
    train(model, mydataloader, loss_function, optimizer, device, batch_size, epochs, eval_dataloader, scheduler, vocab_size, file1)   
    file1.close() 